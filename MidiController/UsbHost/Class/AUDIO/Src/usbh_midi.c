...
USBH_StatusTypeDef USBH_AUDIO_FindMidiStreamingOut(USBH_HandleTypeDef *phost)
{
	...
	for (interface = 0U; interface < USBH_MAX_NUM_INTERFACES; interface++)
	{
		...
		{
			for(int endpoint = 0; endpoint < USBH_MAX_NUM_ENDPOINTS; endpoint++)
			{
				if (((phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].bEndpointAddress & 0x80U) == 0x00U) &&
				  (phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].wMaxPacketSize > 0U))
				{
					AUDIO_Handle->midi_stream_out[alt_settings].Ep = phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].bEndpointAddress;
					AUDIO_Handle->midi_stream_out[alt_settings].EpSize = phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].wMaxPacketSize;
					...
					AUDIO_Handle->midi_stream_out[alt_settings].Poll = phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].bInterval;
	...
	for (interface = 0U; interface < USBH_MAX_NUM_INTERFACES; interface++)
	{
			...
			(phost->device.CfgDesc.Itf_Desc[interface].bInterfaceSubClass == USB_SUBCLASS_MIDISTREAMING))
		{
			for(int endpoint = 0; endpoint < USBH_MAX_NUM_ENDPOINTS; endpoint++)
			{
				...
				(phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].wMaxPacketSize > 0U))
				{
					AUDIO_Handle->midi_stream_in[alt_settings].Ep = phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].bEndpointAddress;
					AUDIO_Handle->midi_stream_in[alt_settings].EpSize = phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].wMaxPacketSize;
					...
					AUDIO_Handle->midi_stream_in[alt_settings].Poll = phost->device.CfgDesc.Itf_Desc[interface].Ep_Desc[endpoint].bInterval;
